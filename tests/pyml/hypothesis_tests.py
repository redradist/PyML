...
class Testing_Hypothesis(unittest.TestCase):
    ...
    def test__LinearHypothesisQuadratic_Theta0Zero__Valid(self):
        hypo = Hypothesis(0, 1, x1=lambda x: pow(x, 2))
        self.assertEqual(hypo(x0=1, x1=2), 4)

    def test__LinearHypothesisQuadraticString_Theta0Zero__Valid(self):
        hypo = Hypothesis(0, 1, equation='th0*x0 + th1*x1', x1=lambda x: pow(x, 2))
        self.assertEqual(hypo(x0=1, x1=2), 4)

    def test__LinearHypothesisQuadratic__Valid(self):
        hypo = Hypothesis(1, 1, x1=lambda x: pow(x, 2))
        self.assertEqual(hypo(x0=1, x1=2), 5)

    def test__LinearHypothesisQuadratic2__Valid(self):
        hypo = Hypothesis(1, 1, x1=lambda x: pow(x, 2))
        self.assertEqual(hypo(1, 2), 5)
    ...
    def test__LinearHypothesisCubic_Theta0Zero__Valid(self):
        hypo = Hypothesis(0, 1, x1=lambda x: pow(x, 3))
        self.assertEqual(hypo(x0=1, x1=2), 8)

    def test__LinearHypothesisCubic__Valid(self):
        hypo = Hypothesis(1, 1, x1=lambda x: pow(x, 3))
